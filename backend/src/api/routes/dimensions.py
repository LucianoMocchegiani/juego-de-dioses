...
from src.models.schemas import DimensionResponse, WorldSizeResponse
...
async def get_world_size():
    ...
    del mundo abierto. Esto es necesario para que el sol y la luna orbiten alrededor
    del mundo completo, no solo de un bloque individual.
    
    Returns:
        Tamaño total del mundo con radio calculado
    """
    async with get_connection() as conn:
        rows = await conn.fetch("""
            SELECT 
        ...
        if not rows:
            # Si no hay bloques, devolver valores por defecto
        ...
        # Calcular bounding box de todos los bloques
        min_x = float('inf')
        max_x = float('-inf')
        min_y = float('inf')
        max_y = float('-inf')
        ...
        # Calcular centro del mundo
        centro_x = (min_x + max_x) / 2.0
        centro_y = (min_y + max_y) / 2.0
        
        # Calcular radio del mundo (distancia desde el centro hasta la esquina más lejana)
