...
import { loadModel3D } from '../../loaders/model-utils.js';
import { listBones, mapBonesToBodyParts, hasSkeleton, listBonesFormatted } from '../../loaders/bones-utils.js';
...
export class PlayerFactory {
     ...
     * @param {Object} [options.charactersApi] - Port de personajes (getCharacter, createCharacter); requerido si se usa characterId o templateId
    ...
    static async createPlayer(options) {
        ...
        try {
            // Si hay characterId, cargar desde API (requiere charactersApi inyectado)
            if (characterId && bloqueId && charactersApi) {
                character = await charactersApi.getCharacter(bloqueId, characterId);
            }
            // Si hay templateId, crear personaje primero (requiere charactersApi inyectado)
            else if (templateId && bloqueId && charactersApi) {
                character = await charactersApi.createCharacter(bloqueId, templateId, x, y, z);
            }
