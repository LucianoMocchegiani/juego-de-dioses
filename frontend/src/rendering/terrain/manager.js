...
export class TerrainManager {
    ...
    constructor(scene, particlesApi, bloquesApi, geometryRegistry, performanceManager = null) {
        ...
        // Ports inyectados (particlesApi, bloquesApi)
        this.particlesApi = particlesApi;
        this.bloquesApi = bloquesApi;

        // Inicializar sistemas
        this.viewportSystem = new ViewportSystem();
        this.styleSystem = new StyleSystem();

        // Sistema de actualización (recibe el port de partículas)
        this.updateSystem = new UpdateSystem(this.particlesApi);
    ...
    async loadParticlesAroundPlayer(playerPositionInMeters) {
        ...
        try {
            ...
            const [particlesData, typesData] = await Promise.all([
                this.particlesApi.getParticles(this.currentDimension.id, viewport),
                this.particlesApi.getParticleTypes(this.currentDimension.id, viewport)
            ]);
