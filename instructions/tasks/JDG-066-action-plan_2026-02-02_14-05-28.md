...
|------|-------------|--------|
| 1 | Dominio bloques — Hexagonal completo | ✅ Hecho |
| 2 | Dominio particles — puerto, adaptador, casos de uso | ✅ Hecho |
| 3 | Dominio characters — puerto, adaptador (list/get/model) | ✅ Hecho |
| 4 | Dominio celestial — casos de uso (state, temperature) | ✅ Hecho |
| 5 | Dominio agrupaciones | ✅ Hecho |
| 6 | world_creation_engine — repos inyectados | ⏸️ Diferido |
| 7 | main.py — registro de dependencias | ✅ No requerido (cada router usa Depends local) |
| 8 | shared y limpieza final | ⏸️ Opcional |
| 9 | Tests unitarios de casos de uso | ⏳ No aplica (no se requieren por ahora) |
| 10 | Documentación y verificación API | ✅ Hecho (READMEs, docs/flujo-endpoints-hexagonal-ddd.md) |
| Final | Generar descripción del PR | ⏳ Pendiente |

**Hecho:** bloques, particles, agrupaciones, characters (list/get/model), celestial (GET/POST) tienen puertos, casos de uso e infrastructure donde aplica; routes sin SQL salvo create_character y background tasks. READMEs y docs actualizados.

**Diferido:** Paso 6 (world_creation_engine) requiere un puerto de escritura (IEntityCreationRepository) y refactor de builders; create_character sigue usando EntityCreator(conn).

---

## Deuda técnica (ejecutar; lo no usado se depreca, no se elimina)

| Ítem | Descripción | Estado |
|------|-------------|--------|
| D1 | Añadir al plan esta sección y pasos | ✅ |
| D2 | Extender IParticleRepository: bloques con partículas no extraídas, partículas con inercia, actualizar temperatura | ✅ |
| D3 | Migrar tarea en background de temperatura (celestial) a usar IParticleRepository; quitar get_connection de la tarea | ✅ |
| D4 | Deprecar en particles.service y celestial.service las funciones reemplazadas (docstring/warning; no eliminar) | ✅ |
| D5 | Documentar WorldBloque: dependencia de celestial.service; deprecar uso directo (preferir inyectar puerto de temperatura) | ✅ |

**Regla:** Lo que deje de usarse se marca como deprecado; posteriormente se **eliminó** el código deprecado (`get_particulas_con_inercia`, `update_particle_temperature`).

**Pasos de implementación:**

1. **D2** — En `IParticleRepository` y `PostgresParticleRepository`: añadir `get_distinct_bloque_ids_for_temperature_update() -> List[str]`, `get_particles_with_thermal_inertia(bloque_id: str, inercia_minima: float = 0.1) -> List[dict]`, `update_particle_temperature(particula_id: str, temperatura: float) -> None`.
2. **D3** — En `celestial/routes.py`: la tarea `update_particle_temperatures_periodically` debe usar una instancia de `PostgresParticleRepository()` (creada al arrancar la tarea) y los métodos anteriores; eliminar `get_connection()` y las llamadas a `get_particulas_con_inercia(bloque_id, conn)` y `update_particle_temperature(..., conn=conn)`; conservar llamada a `calculate_cell_temperature` (celestial.service) para el cálculo.
3. **D4** — En `particles/service.py`: marcar como deprecadas `get_particulas_con_inercia` (docstring: preferir `IParticleRepository.get_particles_with_thermal_inertia`). En `celestial/service.py`: marcar como deprecada `update_particle_temperature` (docstring: preferir `IParticleRepository.update_particle_temperature`).
4. **D5** — En `shared/world_bloque.py`: docstring en `calcular_temperatura` indicando dependencia de `celestial.service.calculate_cell_temperature` y que en el futuro se prefiere inyectar un puerto `ITemperatureCalculator`. Actualizar `shared/README.md` si aplica.

---

## Deuda que queda (anexo al plan)

| Ítem | Dónde | Descripción | Prioridad |
|------|--------|-------------|-----------|
| **D-Q1** | characters/routes.py | create_character. | ✅ Resuelto: ICharacterCreationPort + EntityCreationAdapter; routes sin get_connection. |
| **D-Q2** | particles/service.py | celestial.service dependía de particles.service. | ✅ Resuelto: IParticleRepository.get_particles_near, get_particle_type_by_name; celestial.service recibe particle_repo. |
| **D-Q3** | shared/world_bloque.py | WorldBloque dependía de celestial.service. | ✅ Resuelto: ITemperatureCalculator, CelestialTemperatureCalculatorAdapter, WorldBloque/Manager aceptan temperature_calculator opcional. |

**Regla actual:** `get_connection()` solo en **infrastructure/**, **database/** y **main.py** (seeds). Ningún routes.py usa get_connection (D-Q1, D-Q2, D-Q3 ejecutados).

---

## Cómo resolver la deuda técnica

Orden sugerido: **D-Q2** (desacopla celestial de particles.service) → **D-Q3** (opcional, desacopla WorldBloque) → **D-Q1** (Paso 6, más coste).

### D-Q1 — create_character / world_creation_engine (Paso 6)

**Objetivo:** Que `create_character` no use `get_connection()` ni `EntityCreator(conn)`; que la creación use puertos inyectados.

**Pasos:**

1. **Definir puerto(s) de escritura** para la creación de entidades: por ejemplo `IEntityCreationPort` (o uno por agregado: `ICharacterCreationPort`, etc.) con métodos como `create_character(character_data, bloque_id) -> CharacterResponse` que encapsulen la lógica actual de EntityCreator + builders. Alternativa: que `EntityCreator` reciba en el constructor repositorios/adaptadores (ej. un adaptador que implemente “crear agrupación”, “crear partículas”, “crear personaje”) en lugar de `conn`.
2. **Implementar adaptador(es)** en infrastructure (o en world_creation_engine como capa que use `get_connection()` y SQL/EntityCreator por dentro) que implementen ese puerto.
3. **Refactor de EntityCreator y builders:** sustituir uso de `conn` por llamadas a los repos/adaptadores inyectados (por ejemplo: “crear agrupación” vía repo, “crear partículas” vía repo, etc.). Si se opta por un único puerto `create_character`, el adaptador puede seguir usando EntityCreator internamente con una conexión obtenida en el adaptador.
4. **characters/routes.py:** En `create_character`, inyectar el puerto (p. ej. `Depends(get_entity_creation_port)`) y llamar al caso de uso o al puerto; eliminar `get_connection()` y el `async with get_connection() as conn` en la ruta.

**Archivos típicos:** `world_creation_engine/creators/entity_creator.py`, builders, `characters/routes.py`, nuevo(s) archivo(s) en `application/ports/` y `infrastructure/` para el puerto de creación.

---

### D-Q2 — particles/service.py usado por celestial.service

**Objetivo:** Que `calculate_cell_temperature` no llame a `particles.service`; que use un puerto inyectado (p. ej. `IParticleRepository` extendido). Así `get_connection()` solo queda en **particles/infrastructure**.

**Pasos:**

1. **Extender `IParticleRepository`** (o definir un puerto mínimo en celestial, p. ej. `IParticleDataForTemperature`) con:
   - `get_particles_near(bloque_id, celda_x, celda_y, celda_z, radio) -> List[dict]` (equivalente a `get_particulas_vecinas`: filas con tipo_nombre, temperatura, celda_*, etc.).
   - `get_particle_type_by_name(nombre: str) -> Optional[dict]` (equivalente a `get_tipo_particula_por_nombre`: fila del tipo con conductividad_termica, albedo, etc.).
2. **Implementar** ambos métodos en `PostgresParticleRepository` (mover la lógica SQL desde `particles/service.py`).
3. **Refactor de `celestial/service.py`:** `calculate_cell_temperature(..., particle_repo: IParticleRepository)` (o el puerto definido). En el cuerpo, sustituir las llamadas a `particles.service.get_particulas_vecinas` y `get_tipo_particula_por_nombre` por `particle_repo.get_particles_near` y `particle_repo.get_particle_type_by_name`. No importar ya `particles.service` en ese flujo.
4. **Inyectar el repo** en todos los llamadores de `calculate_cell_temperature`: (a) `celestial/routes.py` — POST temperature: obtener repo vía `Depends(get_particle_repository)` y pasarlo al caso de uso que llama a `calculate_cell_temperature`; (b) tarea en background de temperatura: ya tiene `PostgresParticleRepository()`, pasarlo a `calculate_cell_temperature`; (c) `celestial/application/calculate_temperature.py`: que el caso de uso reciba también `IParticleRepository` y lo pase a `calculate_cell_temperature`.
5. **WorldBloque.calcular_temperatura:** Sigue llamando a `calculate_cell_temperature`; si WorldBloque no tiene acceso al repo, opciones: (i) que quien llame a `calcular_temperatura` pase también el repo (y WorldBloque lo recibe como parámetro), o (ii) resolver D-Q3 inyectando `ITemperatureCalculator` en WorldBloque.
6. **Limpieza:** Si tras esto ninguna ruta ni celestial ni shared usa `get_particulas_vecinas` ni `get_tipo_particula_por_nombre` de `particles.service`, se pueden eliminar o marcar como uso interno; si siguen usándose en otros flujos, dejarlas o migrar esos flujos también.

**Archivos típicos:** `particles/application/ports/particle_repository.py`, `particles/infrastructure/postgres_particle_repository.py`, `celestial/service.py`, `celestial/routes.py`, `celestial/application/calculate_temperature.py`.

---

### D-Q3 — WorldBloque.calcular_temperatura (opcional)

**Objetivo:** Que `WorldBloque` no importe ni llame a `celestial.service.calculate_cell_temperature`; que use un puerto inyectado.

**Pasos:**

1. **Definir puerto** `ITemperatureCalculator` (Protocol o ABC): método `async def calculate_cell_temperature(celda_x, celda_y, celda_z, bloque_id, celestial_time_service, tipo_particula_superficie=None) -> float`. Ubicación: `shared/ports/` o `celestial/application/ports/`.
2. **Implementar adaptador** que delegue en `celestial.service.calculate_cell_temperature` (el adaptador obtiene los datos que necesite; si `calculate_cell_temperature` ya recibe `particle_repo` por D-Q2, el adaptador puede recibir también el repo y pasarlo).
3. **WorldBloque:** Añadir en `__init__` un parámetro opcional `temperature_calculator: Optional[ITemperatureCalculator] = None`. En `calcular_temperatura`, si `self._temperature_calculator` está definido, llamar a `self._temperature_calculator.calculate_cell_temperature(...)`; si no, mantener el comportamiento actual (importar y llamar a `celestial.service.calculate_cell_temperature`) para compatibilidad.
4. **WorldBloqueManager:** Añadir en `__init__` un parámetro opcional `temperature_calculator: Optional[ITemperatureCalculator] = None` y pasarlo a cada `WorldBloque` que cree. Quien instancie `WorldBloqueManager` (si hay algún punto de composición) puede inyectar el adaptador.

**Archivos típicos:** `shared/ports/` (nuevo protocolo), `celestial/` (adaptador que envuelve `calculate_cell_temperature`), `shared/world_bloque.py`, `shared/world_bloque_manager.py`.
